...
class PaymentController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (!empty($id)) {
            ...
            $this->userid     = $user->id;
            $this->api_key    = "hkttruzpxboovmme7s7ua6cppa6m0xp8";
            $this->password   = "xRSLnG38I9uH";
            $this->domain     = "https://api.cryptopaymentchecker.futuregenit.com";
            $this->wallet_url = "/api/address/getwalleteAddress";

            ///api/address/getwalleteAddress?api_key=hkttruzpxboovmme7s7ua6cppa6m0xp8&password=xRSLnG38I9uH
        }
    }
    ...
    public function checkwalletAddress(Request $request)
    {
        ...
        $wallet_url = $this->wallet_url;
        $domain     = $this->domain;

        $url = $this->domain . $wallet_url . "?api_key=" . urlencode($api_key) . "&password=" . urlencode($password);
        ...
        if (curl_errno($ch)) {
            ...
            $response = json_encode(['message' => $errorMessage], JSON_PRETTY_PRINT);
            http_response_code(500); // Set HTTP status code for error
        } else {
            $response = json_encode(['message' => 'Request successful.', 'data' => json_decode($response, true)], JSON_PRETTY_PRINT);
